...
class RegressionAnalysisComplete:
    """
    Used to save regression results and generate plots that are used in the paper
    """
...
class CompareRegressionResults:
    ...
    def block_bootstrap_pooled_coef(self, gtemp, block=5, B=1000):
        """df_* have a column per (state, month) and rows are years."""
        df_
        n_years = len(df_era5)
        years = np.arange(n_years)
        # Pre‑compute block start indices
        starts = np.arange(0, n_years - block + 1)
        pooled = pd.DataFrame(columns=df_era5.columns, index=range(B))
        
        for b in range(B):
            # Draw enough blocks to cover n_years
            idx = []
            while len(idx) < n_years:
                s = np.random.choice(starts)
                idx.extend(years[s:s+block])
            idx = idx[:n_years]                       # truncate to exact length
            
            g   = gtemp.iloc[idx].values
            X   = sm.add_constant(g)                  # [n_years, 2]
            
            # Fit all columns at once for speed
            b_era  = (np.linalg.lstsq(X, df_era5.iloc[idx].values,   rcond=None)[0])[1]
            se_era = df_era5.iloc[idx].std(axis=0) / np.sqrt((g - g.mean())@ (g - g.mean()))
            
            b_mer  = (np.linalg.lstsq(X, df_merra.iloc[idx].values,  rcond=None)[0])[1]
            se_mer = df_merra.iloc[idx].std(axis=0)  / np.sqrt((g - g.mean())@ (g - g.mean()))
            
            w_era, w_mer = 1/se_era**2, 1/se_mer**2
            pooled.loc[b] = (w_era*b_era + w_mer*b_mer) / (w_era + w_mer)

        return pooled          # B × 600 matrix of bootstrapped pooled slopes
...
class PlotlySlopeMap:
    ...
    def __init__(self, csv_path, sig_level: float = 0.05):
        ...
        self.data      = (
            pd.read_csv(self.csv_path)
              .assign(Month=lambda d:
                      pd.Categorical(d["Month"],
                                     categories=self._month_order,
                                     ordered=True))
        )
    ...
    def make_figure(self):
        ...
        fig = choropleth(
            ...
            range_color=[-self._cmax, self._cmax],
            ...
            hover_data={
                "Pooled_Slope":":.3f",
                "Slope_p":":.3f",
                "Region":False   # already shown as location
            },
        )
        fig.update_layout(
            title=("ERA5 + MERRA‑2 pooled temperature sensitivity "
                   "(1980–2022) • slider = month"),
        ...
        # -----------------------------------------------------------------
        # overlay "x" markers on non‑significant states for each frame
        # -----------------------------------------------------------------
        # build Scattergeo traces, one per frame
        frames_dict = {fr.name: fr for fr in fig.frames}
        for month in self._month_order:
            sub = self.data.query("Month == @month and Slope_p >= @self.sig_level")
            if sub.empty:
                continue
            lats  = [self._centroids[s][0] for s in sub["Region"]]
            lons  = [self._centroids[s][1] for s in sub["Region"]]
            ...
            frames_dict[month].data += (marker,)
            fig.add_trace(marker) if month == "Jan" else None

        # ensure first visible frame is January
        # Fix the slider update code
        if hasattr(fig, 'layout') and hasattr(fig.layout, 'sliders') and len(fig.layout.sliders) > 0:
